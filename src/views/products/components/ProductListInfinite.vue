...
import { useProductListInfinite } from '../hooks/ProductList.hook';
...
<template>
  ...
  <template v-else>
    <Button @click="loadNext">Load Next Page</Button>
    <Shimmer v-if="productsLoading" class="product-shimmer" animate />
    <EmptyState
      v-else-if="!temp_products.length"
      :image="NotFound"
    ...
    <div v-else class="product-card-grid">
      <Card
        class="product-card"
        :key="product.id"
        v-for="product of temp_products"
