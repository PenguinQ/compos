export const PRODUCT_LIST = {
  PRODUCT_EMPTY_TITLE: 'Hmm...',
  PRODUCT_EMPTY_DESCRIPTION: 'Looks like you doesn\'t have any products, try adding a new one!',
  BUNDLE_EMPTY_TITLE: 'Hmm...',
  BUNDLE_EMPTY_DESCRIPTION: 'Looks like you doesn\'t have any bundles, try adding a new one!',
  SEARCH_EMPTY_TITLE: 'Welp.',
  SEARCH_EMPTY_DESCRIPTION: 'There\'s no products/bundles with that name, try another keyword perhaps?',
};

export const PRODUCT_DETAIL = {
  ERROR_TITLE: 'Uhm...',
  ERROR_DESCRIPTION: 'Failed to get product detail, please try again.',
  EMPTY_VARIANT_TITLE: 'Hmm...',
  EMPTY_VARIANT_DESCRIPTION: 'This product doesn\'t have any variants.',
};

export const PRODUCT_FORM = {
  EMPTY_VARIANT_TITLE: 'Hmm...',
  EMPTY_VARIANT_DESCRIPTION: 'Looks like you doesn\'t have any variants for this product, try adding a new one!',
};

export const BUNDLE_DETAIL = {
  EMPTY_PRODUCT_TITLE: 'Hmm...',
  EMPTY_PRODUCT_DESCRIPTION: 'This bundle doesn\'t have any products.',
};

export const BUNDLE_FORM = {
  PRODUCT_EMPTY_TITLE: 'Hmm...',
  PRODUCT_EMPTY_DESCRIPTION: 'Looks like you doesn\'t have any products for this bundle, try adding a new one!',
  PRODUCT_LIST_EMPTY_TITLE: 'Hmm...',
  PRODUCT_LIST_EMPTY_DESCRIPTION: 'Looks like there\'s no active products, try adding new product or update the existing one.',
  PRODUCT_SEARCH_EMPTY_TITLE: 'Welp.',
  PRODUCT_SEARCH_EMPTY_DESCRIPTION: 'There\'s no active products with that name, try another keyword perhaps?',
  PRICE_MESSAGE: 'The price automatically follows the sum of all products in the products list.',
  AUTO_PRICE_MESSAGE: 'Uncheck this box if you want to set the price to your preferences.'
};
