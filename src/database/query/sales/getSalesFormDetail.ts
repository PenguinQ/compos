...
export type SalesFormDetailProduct = {
  id: string;
  images: string[];
  name: string;
  quantity: number;
...
type SalesFormDetailQuery = Omit<QueryParams, 'limit' | 'observe' | 'page' | 'sort'> & {
  id: string;
};

export default async ({ id, normalizer }: SalesFormDetailQuery) => {
  try {
